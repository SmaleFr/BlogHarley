<template>
  <div class="max-w-7xl mx-auto px-4 py-12">
    <NuxtLink to="/blog" class="text-harley-orange hover:underline text-sm">← Retour au blog</NuxtLink>

    <div class="mt-4 mb-8">
      <h1 class="text-3xl font-bold" v-if="category">{{ category.name }}</h1>
      <p class="text-gray-500 mt-1" v-if="category">{{ category.description }}</p>
    </div>

    <div class="flex flex-wrap gap-2 mb-8">
      <NuxtLink to="/blog" class="px-4 py-2 rounded-full text-sm font-medium border transition border-gray-300 text-gray-600 hover:border-harley-orange">
        Tous
      </NuxtLink>
      <NuxtLink
...
const category = computed(() => categories.value?.find((c: any) => c.slug === route.params.slug))
const { data: articles } = await useFetch(() => `/api/blog?categorie=${route.params.slug}`)
</script>
